...
def print_statistics(results: dict):
    ...
    print(
        f"  beam_amp hitting upper bound (≥49.9): {np.sum(free[:, 2] >= 49.9)} ({100 * np.sum(free[:, 2] >= 49.9) / len(free):.1f}%)"
    )
    ...
    for strategy, label in zip(strategies, labels, strict=False):
        ...
        if strategy == "high_bound":
            print(
                f"  beam_amp hitting new upper bound (≥99.9): {np.sum(data[:, 2] >= 99.9)} ({100 * np.sum(data[:, 2] >= 99.9) / len(data):.1f}%)"
            )
            print(
                f"  beam_amp > 50: {np.sum(data[:, 2] > 50)} ({100 * np.sum(data[:, 2] > 50) / len(data):.1f}%)"
            )
...
def aggregate_results(all_results: list) -> dict:
    ...
    aggregated = {"free": [], "fixed_25": [], "no_beam": [], "high_bound": []}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-dates", type=int, default=15, help="Number of random dates to test"
    )
    parser.add_argument(
        "--chunks-per-date", type=int, default=50, help="Number of chunks per date"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("scratch/beam_amp_sensitivity.png"),
