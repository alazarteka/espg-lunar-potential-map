...
def plot_azimuthal_power_distribution(
    ...
    for i, (m, count) in enumerate(zip(m_values, mode_counts)):
        if i % 2 == 0 or count > 0:  # Label every other or if has modes
            axes[0, 0].text(
                m, power_by_m[i], f'n={count}',
    ...
    for m in m_to_plot:
        if m < len(m_values):
            ...
            axes[1, 0].plot(hours, power_temporal[:, m], 'o-', label=label, alpha=0.7, markersize=3)
...
def plot_degree_order_spectrum(
    ...
    ax.plot([0, lmax], [0, lmax], 'r--', alpha=0.5, linewidth=2, label='Sectoral (l=|m|)')
    
    # Add vertical line at m=0 (zonal harmonics)
    ax.axvline(0, color='cyan', linestyle='--', alpha=0.5, linewidth=2, label='Zonal (m=0)')
...
def print_mode_statistics(analysis: dict, lmax: int) -> None:
    ...
    print("-" * 70)
    print(f"{'|m|':<5} {'Modes':<8} {'Power (V)':<12} {'% Total':<10} {'Power/Mode':<12}")
    print("-" * 70)
    
    for m, count, power in zip(m_values, mode_counts, power_by_m):
        pct = (power / total_power * 100) if total_power > 0 else 0
        per_mode = power / count if count > 0 else 0
        label = "ZONAL" if m == 0 else ""
        print(f"{m:<5} {count:<8} {power:<12.1f} {pct:<10.1f} {per_mode:<12.1f} {label}")
    ...
    print(f"Non-zonal (m≠0) power: {nonzonal_power:8.1f} V ({nonzonal_power/total_power*100:5.1f}%)")
