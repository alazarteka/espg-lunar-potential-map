...
def plot_coefficient_comparison(
    ...
    for row, (idx, label) in enumerate(zip(indices_to_plot, mode_labels)):
        if idx >= coeffs_ind.shape[1]:
            continue
        
        # Real part
        axes[row, 0].plot(
            hours_ind, coeffs_ind[:, idx].real, 'o-',
            label='Independent', alpha=0.7, markersize=4
        )
        axes[row, 0].plot(
            hours_coup, coeffs_coup[:, idx].real, 's-',
            label='Coupled', alpha=0.7, markersize=4
        )
        ...
        # Imaginary part
        axes[row, 1].plot(
            hours_ind, coeffs_ind[:, idx].imag, 'o-',
            label='Independent', alpha=0.7, markersize=4
        )
        axes[row, 1].plot(
            hours_coup, coeffs_coup[:, idx].imag, 's-',
            label='Coupled', alpha=0.7, markersize=4
        )
...
def plot_temporal_roughness(
    ...
    axes[0].plot(
        hours_mid_ind, roughness_ind, 'o-',
        label='Independent', alpha=0.7, markersize=4
    )
    axes[0].plot(
        hours_mid_coup, roughness_coup, 's-',
        label='Coupled', alpha=0.7, markersize=4
    )
    ...
    bins = np.linspace(
        0, max(roughness_ind.max(), roughness_coup.max()), 30
    )
    axes[1].hist(
        roughness_ind, bins=bins, alpha=0.6,
        label=f'Independent (mean={roughness_ind.mean():.1f} V)', color='C0'
    )
    axes[1].hist(
        roughness_coup, bins=bins, alpha=0.6,
        label=f'Coupled (mean={roughness_coup.mean():.1f} V)', color='C1'
    )
...
def plot_residual_comparison(
    ...
    # Histogram comparison
    bins = np.linspace(
        min(rms_ind.min(), rms_coup.min()),
        max(rms_ind.max(), rms_coup.max()),
        25
    )
    axes[1].hist(
        rms_ind, bins=bins, alpha=0.6,
        label=f'Independent (median={np.median(rms_ind):.1f} V)', color='C0'
    )
    axes[1].hist(
        rms_coup, bins=bins, alpha=0.6,
        label=f'Coupled (median={np.median(rms_coup):.1f} V)', color='C1'
    )
...
def print_summary_statistics(
    ...
    print(f"\nTemporal Roughness:")
    print(f"  Independent: mean={roughness_ind.mean():.2f} V, std={roughness_ind.std():.2f} V")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("plots/temporal_comparison"),
