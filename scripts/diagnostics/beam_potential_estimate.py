...
def main() -> int:
    ...
    print(
        f"Detection: min_peak={args.min_peak}, energy=[{args.energy_min}, {args.energy_max}] eV"
    )
    print()
    ...
    for chunk_idx in range(n_chunks):
        ...
        u_surface_estimate = args.u_spacecraft - detection.beam_energy
        ...
        if args.verbose:
            print(
                ...
                f"beam={detection.beam_energy:6.1f} eV, "
                f"amp={detection.peak_value:4.2f}, "
    ...
    print(
        f"  Range:  {np.min(u_surface_values):.1f} – {np.max(u_surface_values):.1f} V"
    )
