"""
Refactored Lunar Prospector Surface Potential Mapper

This module provides a clean, modular interface for calculating and visualizing
surface potentials from Lunar Prospector electron reflectometer data.
"""
...
class DataLoader:
    ...
    def discover_flux_files(data_directory: Union[str, Path], 
                           ...
                           months: Optional[List[str]] = None) -> List[Path]:
...
class CoordinateCalculator:
    ...
    def calculate_coordinate_arrays(self, flux_data: FluxData) -> CoordinateArrays:
        ...
        for i, utc_time in enumerate(flux_data.data['UTC']):
            ...
            ra, dec = get_current_ra_dec(time, self.et_spin, self.ra_vals, self.dec_vals)
        ...
        unit_vectors_to_sun = lp_vectors_to_sun / np.linalg.norm(lp_vectors_to_sun, axis=1, keepdims=True)
        scd_to_j2000_mats = build_scd_to_j2000(ra_dec_cartesian, unit_vectors_to_sun)
        scd_to_iau_moon_mats = np.einsum("nij,njk->nik", j2000_to_iau_moon_mats, scd_to_j2000_mats)
...
class MagneticFieldProcessor:
    ...
    @staticmethod
    def project_magnetic_field(flux_data: FluxData, coord_arrays: CoordinateArrays) -> np.ndarray:
        ...
        projected_magnetic_field = np.einsum("nij,nj->ni", 
                                           coord_arrays.scd_to_iau_moon_mats, 
                                           unit_magnetic_field)
...
class SurfaceIntersectionFinder:
    ...
    @staticmethod
    def find_intersections(coord_arrays: CoordinateArrays, 
                          projected_magnetic_field: np.ndarray,
                          potentials: np.ndarray) -> PotentialResults:
        ...
        for i, (position, mag_field, moon_vector_to_sun) in enumerate(
            zip(coord_arrays.lp_positions, projected_magnetic_field, coord_arrays.moon_vectors_to_sun)
        ):
...
class PotentialMapper:
    ...
    def __init__(self, data_directory: Union[str, Path], spice_directory: Union[str, Path]):
    ...
    def process_flux_file(self, file_path: Union[str, Path]) -> PotentialResults:
        ...
        projected_magnetic_field = MagneticFieldProcessor.project_magnetic_field(flux_data, coord_arrays)
    ...
    def create_potential_map(self, results_list: List[PotentialResults], 
        ...
        all_intersections = np.concatenate([r.intersection_positions for r in results_list])
        ...
        # Normalize vectors
        all_facing_sun = all_facing_sun / np.linalg.norm(all_facing_sun, axis=1, keepdims=True)
        all_intersections = all_intersections / np.linalg.norm(all_intersections, axis=1, keepdims=True)
        ...
        sc = plt.scatter(longitudes_plot, latitudes_plot, c=potentials_plot, 
...
Examples:
  ...
  python potential_mapper.py --output potential_map.png
    ...
    parser.add_argument(
        ...
        nargs='+', 
        type=str,
        help='Specific flux files to process'
    )
    ...
    parser.add_argument(
        ...
        nargs='+', 
        type=str,
        help='Years to process (e.g., 1998 1999)'
    )
    ...
    parser.add_argument(
        ...
        nargs='+', 
        type=str,
        help='Months to process (e.g., 01 02 03)'
    )
