...
ELECTRON_CHARGE_mag = config.ELECTRON_CHARGE.magnitude  # Charge of an electron in Coulombs
CHARGE_PER_ELECTRON = ELECTRON_CHARGE_mag * ureg.coulomb / ureg.particle  # Charge per electron in Coulombs

def electron_current_density(params: KappaParams,
    ...
    energies = np.geomspace(E_min.magnitude, E_max.magnitude, num=n_steps) * ureg.electron_volt
    F_omni = omnidirectional_flux_magnitude(density_mag=params.density.magnitude,
                                            kappa=params.kappa,
                                            theta_mag=params.theta.magnitude,
                                            energy_mag=energies.magnitude) * (ureg.particle / (
        ureg.centimeter**2 * ureg.second * ureg.electron_volt))

    energies_mag = energies.magnitude
    F_omni_mag = F_omni.to(ureg.particle / (ureg.meter**2 * ureg.second * ureg.electron_volt)).magnitude

    number_flux = 0.25 * simpson(F_omni_mag, energies_mag) * (ureg.particle / (
