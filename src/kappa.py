...
from src.physics.kappa import KappaParams, omnidirectional_flux, omnidirectional_flux_integrated, velocity_from_energy
...
class Kappa:
    ...
    def __init__(
        ...
        self.solid_angles = np.loadtxt(
            config.DATA_DIR / config.SOLID_ANGLES_FILE,
            dtype=np.float64,
        ).reshape(-1, config.CHANNELS) * ureg.steradian
    ...
    def _prepare_data(self):
        ...
        self.omnidirectional_differential_particle_flux = np.sum(masked_electron_flux * self.solid_angles, axis=1) # shape (Energy Bins,)
    ...
    def _objective_function(self, kappa_theta: np.ndarray) -> float:
        ...
        log_model_differential_flux = np.log(model_differential_flux.to(ureg.particle / (ureg.centimeter**2 * ureg.second * ureg.electron_volt)).magnitude)
        log_data_flux = np.log(self.omnidirectional_differential_particle_flux.to(ureg.particle / (ureg.centimeter**2 * ureg.second * ureg.electron_volt)).magnitude)
    ...
    def _get_density_estimate(self) -> NumberDensity:
        ...
        """
        if self.omnidirectional_differential_particle_flux is None or self.energy_centers is None or self.energy_windows is None:
        ...
        delta_energy = self.energy_windows[:, 1] - self.energy_windows[:, 0]  # shape (Energy Bins,)

        integrand = self.omnidirectional_differential_particle_flux / velocities # shape (Energy Bins,)
    ...
    def fit(self, n_starts: int = 50):
        if not self.is_data_valid:
            raise ValueError("Data is not valid. Ensure that the data has been prepared.")
        ...
        if best_result.success and hasattr(best_result, 'hess_inv'):
            try:
                ...
                sigma = KappaParams(
                    density=0 * ureg.particle / ureg.meter**3,  # Placeholder, density is not estimated from Hessian
                    kappa=sigma[0],
